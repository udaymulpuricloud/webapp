...
public class ExceptionHandling {
    ...
    public ResponseEntity<Void> handleResourceNotFound(ChangeSetPersister.NotFoundException exception){
        HttpHeaders headers = new HttpHeaders();
        headers.set("Pragma", "no-cache");
        headers.set("X-Content-Type-Options","nosniff");
        return ResponseEntity.status(404).cacheControl(CacheControl.noCache().mustRevalidate()).headers(headers).build();
    }
}
